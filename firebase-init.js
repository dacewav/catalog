...
let _allowedEmails = [..._BOOTSTRAP_ADMINS];

// Firebase keys can't contain ".", "#", "$", "/", "[", "]" — emails have dots
function _emailToKey(email) { return email.replace(/\./g, ','); }
function _keyToEmail(key) { return key.replace(/,/g, '.'); }
...
function addWhitelistEmail() {
  ...
  const obj = {}; _allowedEmails.forEach(e => obj[_emailToKey(e)] = true);
...
window.addEventListener('load', () => {
  ...
  firebase.database().ref('adminWhitelist').once('value', snap => {
    ...
    if (fbObj && typeof fbObj === 'object') {
      _allowedEmails = Object.keys(fbObj).map(_keyToEmail);
    } else {
      // Bootstrap admins seed the whitelist
      const user = _auth.currentUser;
      if (user && _BOOTSTRAP_ADMINS.includes(user.email)) {
        const obj = {}; _BOOTSTRAP_ADMINS.forEach(e => obj[_emailToKey(e)] = true);
        firebase.database().ref('adminWhitelist').set(obj);
      }
    }
  });
